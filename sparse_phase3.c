...
sparse_phase3_t* sparse_phase3_encode(const int8_t *vector, size_t dimension) {
    ...
    if (count == 0) {
        sparse_phase3_t *result = malloc(sizeof(sparse_phase3_t));
    ...
    /* Position gaps with ADAPTIVE Rice (Phase 3 optimization) */
    uint16_t prev_gap = 16;  /* Initialize with typical gap */
    for (uint16_t i = 1; i < count; i++) {
        uint16_t gap = positions[i] - positions[i-1] - 1;

        /* Compute adaptive Rice parameter from previous gap */
        uint8_t r = 4;  /* Default */
        if (prev_gap > 0) {
            r = 0;
            uint16_t temp = prev_gap;
            while (temp > 0) {
                temp >>= 1;
                r++;
            }
            r = r > 0 ? r - 1 : 0;  /* log2(prev_gap) */
            if (r < 2) r = 2;
            if (r > 6) r = 6;
        }
...
int sparse_phase3_decode(const sparse_phase3_t *encoded,
    ...
    /* Read positions with ADAPTIVE Rice (Phase 3) */
    ...
    /* Position gaps with adaptive Rice (same logic as encoder) */
    uint16_t prev_gap = 16;  /* Initialize with same value as encoder */
    for (uint32_t i = 1; i < count; i++) {
        /* Compute adaptive Rice parameter (same as encoder) */
        uint8_t r = 4;
        if (prev_gap > 0) {
            r = 0;
            uint16_t temp = prev_gap;
