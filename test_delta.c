 ...
 * test_delta.c
 *
 * Test delta-encoded alphabet (Phase 1 optimization)
 * Expected: ~173 bytes for 97 nz vectors (vs 209 baseline)
...
int main(void) {
    printf("=== Phase 1: Delta-Encoded Alphabet ===\n\n");
    ...
    for (int i = 0; i < 20; i++) {
        ...
        /* Encode with delta-alphabet */
        sparse_delta_t *enc = sparse_delta_encode(vector, DIMENSION);
        ...
        /* Decode */
        int decode_result = sparse_delta_decode(enc, decoded, DIMENSION);
    ...
    if (success_count > 0) {
        ...
        /* Compare to baseline */
        double baseline = 209.0;  /* sparse_optimal_large baseline */
        double savings = baseline - avg_size;
        double improvement = 100.0 * savings / baseline;

        printf("\nComparison:\n");
        printf("  Baseline (sparse_optimal_large): %.0f bytes\n", baseline);
        ...
        printf("\n");
        if (avg_size < 180.0) {
            printf("✓ Target achieved! (expected ~173 bytes)\n");
        } else {
            printf("⚠ Above target (expected ~173 bytes, got %.1f bytes)\n", avg_size);
        }
    }
